...
That's the full minimal config. The chart infers your namespace from the ArgoCD release (one chart instance per namespace) and enables the default alert set with the default severities and thresholds shown in the [standard alert set](#standard-alert-set) table above. Commit and push, and ArgoCD picks it up on the next sync.
...
standardAlerts:
  ...
  # ArgoCD apps often deploy slowly in prod, so give them longer
  argocdAppUnhealthy:
    forDuration: 30m
...
- **Fix or mitigate** the issue so the condition clears, and the alert auto-resolves.
- **Silence it in Grafana** with the Silence link on the Slack message (or Alerting → Silences) for a specific window if you're deliberately ignoring it (planned maintenance, known-bad test env, etc.).
...
- **Namespaces in the same cluster** share a Grafana and its contact points. Just point their `contactPointRef` at the same contact point in your values files. No IDP action is needed, as long as that contact point already exists. Example: if `myteam-dev` and `myteam-test` both live in `idp-shared-test`, setting `contactPointRef: "Slack - MyTeam nonprod"` in both values files routes both namespaces' alerts to the same channel.
...
**Alert rule group stays in error state after a contact-point change.** The grafana-operator can cache provisioning errors after a contact-point rename or re-point, and won't recover on its own even once the underlying issue is fixed. Contact the IDP team, and we can force a re-reconcile cluster-side.
